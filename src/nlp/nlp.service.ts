...
export class NlpService {
  ...
  constructor() {
    ...
    this.sentenceAnalyzer = new natural.SentimentAnalyzer(
      'English',
  ...
  analyzeSentenceStructure(tags: string[]): number {
    ...
    const hasVerb =
      ...
      tags.includes('VBZ');
    const hasAdjective =
      tags.includes('JJ') || tags.includes('JJR') || tags.includes('JJS');
    const hasAdverb =
      tags.includes('RB') || tags.includes('RBR') || tags.includes('RBS');
  ...
  evaluateVocabulary(text: string): number {
    ...
    const advancedWordSet = new Set(
      ADVANCED_WORDS.map((word) => this.stemmer.stem(word)),
    );
    const stemmedWords = words.map((word) => this.stemmer.stem(word));
    const advancedWordCount = stemmedWords.filter((word) =>
      advancedWordSet.has(word),
    ).length;
